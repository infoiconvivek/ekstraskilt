                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Gallery List</h4>
                            <div class="flex-shrink-0">
                                <a href="{{url('admin/design-gallery/create')}}" class="btn btn-info">Add Gallery</a>
                            </div>
                        </div><!-- end card header -->
                                ...
                                <div class="table-responsive pb-4">
                                    <table class="table align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($galleries as $key=>$row)
                                            <tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
                                                <td>{{$key+1+($galleries->currentPage()-1) * ($galleries->perPage())}}</td>
                                                <td>{{$row->title ?? ''}}</td>
                                                <td>{{ ($row->type == 1) ? 'Image' : 'Background' }} </td>
                                                ...
                                                <td>
                                                    <a class="btn btn-info btn-sm waves-effect waves-light" href="{{url('admin/design-gallery/edit/'.$row->id)}}">
