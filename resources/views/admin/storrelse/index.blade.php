                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Storrelse List</h4>
                            <div class="flex-shrink-0">
                                <a href="{{url('admin/storrelse/create')}}" class="btn btn-info">Add Storrelse</a>
                            </div>
                        </div><!-- end card header -->
                                ...
                                <div class="table-responsive pb-4">
                                    <table class="table align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($storrelses as $key=>$row)
                                            <tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
                                                <td>{{$key+1+($storrelses->currentPage()-1) * ($storrelses->perPage())}}</td>
                                                <td>{{$row->bredde ?? ''}} X {{$row->hoyde ?? ''}} mm</td>
                                                ...
                                                <td>
                                                    <a class="btn btn-info btn-sm waves-effect waves-light" href="{{url('admin/storrelse/edit/'.$row->id)}}">
